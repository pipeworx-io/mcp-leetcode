...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/user_profile \
  -H 'Content-Type: application/json' \
  -d '{"username":"john_doe"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/user_profile`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
